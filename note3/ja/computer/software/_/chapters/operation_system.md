...
**メモリリーク**は、主記憶装置読み込まれたプログラムの実行完了後に使用していた領域を解放する際、メモリがただしく解放されないこと。メモリリークが発生すると使えるメモリが少なくなってしまい、動作が不安定になる。

### ガベージコレクション

**ガベージコレクション**（**ガベージコレクタ**）は、メモリリークを防ぐために、利用されていないメモリ領域を解放するプログラム。
