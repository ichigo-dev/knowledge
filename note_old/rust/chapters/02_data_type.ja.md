...
Rustは、IEEE標準準拠の**単精度**と**倍精度**の浮動小数点をサポートしている。 `f32` と `f64` はそれぞれ、IEEE規格が要求する[型関連定数](./08_struct.ja.md#型関連定数)である `INFINITY` 、 `NEG_INFINITY` 、 `NAN` 、 `MIN` と `MAX` を持つ。
...
![Vectorのメモリレイアウト](../images/memory_layout_vec.jpg)
