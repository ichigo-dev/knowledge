...
**ショートサーキット評価**（**短絡評価**)は、[論理演算](#論理演算)において、左辺を評価した段階で式全体の値が定まらない場合のみ右辺を評価する方法。例として、[論理積](#論理演算)の左辺が `false` であれば式全体は必ず `false` になり、[論理和](#論理演算)の左辺が `true` であれば式全体は必ず `true` になることから、右辺は評価する必要がない。このような性質を利用して評価を途中で打ち切ることで、効率的に[プログラム](./programming.md#プログラム)を実行するのがこの方法を利用する利点である。
