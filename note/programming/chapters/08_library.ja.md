...
パッケージは一度リリースされた後も、開発者によってメンテナンスや機能追加が継続的に行われていく場合が多い。同じ名前のパッケージの新旧を区別する番号などのことを**バージョン**という。

パッケージのバージョンが上がっても以前利用できていた機能が継続的に利用できることを**後方互換性**があるという。後方互換性を失ったパッケージは、依存関係を大きく侵害してしまう恐れがあるため注意が必要である。
...
他にも**正式版**のパッケージがリリースされる前に、機能が足りていない試作品として**アルファ版**や、一部の機能が足りておらず動作が不安定である**ベータ版**として試験的に公開される場合もある。
...
**パッケージマネージャ**はプログラムが必要とするパッケージやそのバージョンを管理したり、パッケージの導入を容易にしたりするためのツールである。
