...
**Gitホスティングサービス**はGitが導入されたサーバを提供するサービス。中央サーバを自身で用意することなく、Gitを利用した複数人での開発などを手軽に始めることができる。

代表的なGitホスティングサービスは以下の通り。
...
**ブランチ**はコミットの履歴を枝分かれさせる機能のことで、枝分かれしたブランチは他のブランチとは独立した環境を持つことができる。Git管理の本流となるブランチはデフォルトでは `main` （以前までは `master` ）であり、分岐するブランチには任意の名前をつけることができる。また、分岐したブランチを別のブランチに合流させることを**マージ**という。
