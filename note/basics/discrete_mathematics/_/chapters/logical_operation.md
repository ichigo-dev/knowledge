...
**真理値表**は、[論理演算](#論理演算)の入力の組み合わせと出力を表で表現したもの。例えば、入力を $x$ 、出力を $z$ とし、 $x$ を反転した値を出力するような[論理演算](#論理演算)についての真理値表は以下のように表される。
...
**OR演算**（**論理和**）は、2つの[真偽値](./set_and_proposition.md#真偽値)の入力値のうち、どちらか一方でも真であれば真を出力する演算で、数学的には[和集合](./set_and_proposition.md#和集合)に対応する。
...
**AND演算**（**論理積**）は、2つの[真偽値](./set_and_proposition.md#真偽値)の入力値のうち、両方が真であるときにだけ真を出力する演算で、数学的には[積集合](./set_and_proposition.md#積集合)に対応する。
...
**NOT演算**（**論理否定**）は、1つの[真偽値](./set_and_proposition.md#真偽値)の入力値を反転した値を出力する演算で、数学的には[補集合](./set_and_proposition.md#補集合)に対応する。
...
**NOR演算**（**否定論理和**）は、[OR演算](#or演算)と[NOT演算](#not演算)を組み合わせたもので、2つの[真偽値](./set_and_proposition.md#真偽値)の入力値のうち、どちらか一方でも真であれば偽を出力する演算。
...
**NAND演算**（**否定論理積**）は、[AND演算](#and演算)と[NOT演算](#not演算)を組み合わせたもので、2つの[真偽値](./set_and_proposition.md#真偽値)の入力値のうち、両方が真であるときにだけ偽を出力する演算。
...
**XOR演算**（**排他的論理和**）は、2つの[真偽値](./set_and_proposition.md#真偽値)の入力値の値が異なる場合にのみ真を出力する演算で、数学的には[対称差集合](./set_and_proposition.md#対称差集合)に対応する。
...
これは複雑な[論理演算](#論理演算)を簡略化したり、論理的な等価性を証明したりするために用いられる。
