...
**ルーティング**（**経路制御**）は、宛先IPアドレスのホストまでパケットを届ける仕組み。ルータは正しい方向へパケットを転送するために、ルーティングテーブルを参照してパケットの転送先を決定する。
...
**ネットワークACL**は、システムやファイル、ネットワーク上のリソースなどへのアクセス可否の設定をリストとして列挙したもの。宛先と送信元のIPアドレスおよびポート番号を条件とした上で、その条件に合致した通信の可否をACLとして設定する。
