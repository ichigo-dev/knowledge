...
**単一障害点**(**SPOF**: Single Point Of Failure)は、[システム](./system.md#システム)を構成する要素のうちそこが停止すると[システム](./system.md#システム)全体が停止してしまう部分。[可用性](#可用性)の高い[システム](./system.md#システム)を構成するには、個々の構成要素を[冗長化](#冗長化)してフェイルオーバの機構を用意しておくなど、単一障害点を減らすことが重要となる。
