...
例えば、セミコロン区切りでデータが記載された `data.txt` をCSVとして出力したい場合は、以下の[コマンド](./basic_command.md#コマンド)を実行する。

```sh
$ cat data.txt | tr ; ,
```

`-d` [オプション](./basic_command.md#オプション)を使用すると、指定された文字を削除することができる。
...
例えば、 `test1.txt` と `test2.txt` の差分を確認したい場合は、以下の[コマンド](./basic_command.md#コマンド)を実行する。
...
`-u` [オプション](./basic_command.md#オプション)を付与すると、[ファイル](../../../software/_/chapters/file_system.md#ファイル)の差分をユニファイド出力形式で出力する。
...
例えば、 `old.txt` と `new.txt` の差分をパッチとして `new.txt` に適用したい場合は、以下の[コマンド](./basic_command.md#コマンド)を実行する。
...
例えば、 `test.txt` のテキスト中にある `aaa` という文字列を `bbb` に置換したい場合は次の[コマンド](./basic_command.md#コマンド)を実行する。

```sh
$ sed 's/aaa/bbb/g' test.txt
```

[コマンド](./basic_command.md#コマンド)としては他にも次のようなものがある。
