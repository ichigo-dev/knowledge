...
**負荷分散**は演算処理を複数台のコンピュータ間で分散させること。ネットワークにおいては処理を行うコンピュータの前のレイヤに**ロードバランサ**を設置し、外部からのアクセスを後段のコンピュータに振り分ける。
...
[冗長化](#冗長化)されているシステムにおいて、障害発生時に運用系から待機系のシステムに自動的に切り替える機能のことを**フェイルオーバー**という。フェイルオーバー機能が組み込まれたシステムは[可用性](#rasとrasis)や[信頼性](#rasとrasis)が高いといえる。
